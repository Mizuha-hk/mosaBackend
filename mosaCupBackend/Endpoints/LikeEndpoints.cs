...
public static class LikeEndpoints
{
    public static void MapLikeEndpoints (this IEndpointRouteBuilder routes)
    {
        ...
        //Get Like User
        group.MapGet("/{pid}", async Task<Results<Ok<List<like>>, NotFound>> (Guid Pid, mosaCupDbContext db) =>
        {
        ...
        //Like
        group.MapPost("/", async (LikeReq reqData, mosaCupDbContext db) =>
        {
        ...
        //Delete Like
        group.MapPost("/Delete", async Task<Results<Ok, NotFound>> (LikeReq reqData, mosaCupDbContext db) =>
        {
